...
}, nullptr, &parser::maps::gEquipmentSubparser);
...
void Weapon::load(void) {
	...
	std::stringstream dataStream; dataStream << data.getData();
	std::string lineString;

	while(getline(dataStream, lineString)) {
		if(lineString.length() <= 0) continue;

		std::stringstream line; line << lineString;
		std::string opener;
		...
		if(line >> opener && opener == "weapon") {
			...
			while(getline(dataStream, lineString)) {
				...
				if(!(line >> opener)) break;	
				
				if(!parser::maps::gWeaponSubparser.contains(opener)) {
					parser::invoke_defaults(parser::maps::gWeaponSubparser,
							weapon, lineString, error);
					continue;
				}

				auto& invoker = parser::maps::gWeaponSubparser[opener];
				if(invoker) invoker(weapon, line, error);
				else std::cout << error << ": No weapon property named '" << opener << "'\n";
			}
		}
	}	
