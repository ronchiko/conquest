...
public:
	...
	const char* getName(void) const override { return name; }

	void setStatModifer(Stats stat, int mod) { statTable[stat] = mod; }
	void setGrowthRate(Stats stat, float mod) { growthTable[stat] = mod; }
	int getStatModifer(Stats stat) const { return statTable[stat]; }
	float getGrowthRate(Stats stat) const { return growthTable[stat]; }
private:
...
public:
	...
	void setProffesion(Proffesion* p) { proffesion = p; }
	void setRace(Race* r) { race = r; }
	void setElement(Element* elem) { element = elem; }
	
	const Element* getElement(void) const { return element; }
	const Race* getRace(void) const { return race; }
	const Proffesion* getProffesion(void) const { return proffesion; }
private:
	int level = 1;
...
public:
	...
	T* getSelected(void) { return m_Collection[m_Selected]; }
private:
